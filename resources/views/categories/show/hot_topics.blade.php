<div class="container max-w-6xl p-6 mx-auto space-y-4">
    <p class="flex items-center gap-3 mb-4 text-sm text-gray-400">
        ...
        @foreach($category->ancestors()->breadthFirst()->get() as $ancestor)
            ...
            <a class="text-blue-500" href="{{ url(app()->getLocale().'/categories/'.$ancestor->slug) }}">{{ $ancestor->title }}</a>
        @endforeach
    </p>
    ...
    <ul class="flex flex-wrap gap-2">
        ...
        <li>
            <a type="button" href="{{ url(app()->getLocale().'/categories/'.$descendant->slug) }}" class="px-3 py-1 transition-colors bg-gray-200 rounded hover:bg-gray-300">
