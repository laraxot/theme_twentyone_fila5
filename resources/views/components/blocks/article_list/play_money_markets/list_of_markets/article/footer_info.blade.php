<div class="flex items-center justify-between px-6 lg:px-0 -mt-6 ">
    <div class="flex items-center gap-4 lg:gap-8">
        <div class="relative" x-data="{ tooltip: false }">
            ...
            <div class="flex items-center gap-1.5 text-black">
                ...
                <span {{-- x-text="market.wagers_count_total" --}} class="text-sm text-[#666666]">{{ $article->betting_users }}</span>
            </div>
            <div class="flex items-center gap-1.5 text-black">
                <x-heroicon-o-chart-bar class="size-4"/>
                <span {{-- x-text="formatCurrency(market.volume_play_money)+'ø'" --}}
                    class="text-sm text-[#666666]">{{ $article->volume_credit }}ø</span>
            </div>
            </button>
            <!-- tooltip -->
